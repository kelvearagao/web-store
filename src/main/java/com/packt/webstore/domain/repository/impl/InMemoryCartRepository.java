	...
	public Cart create(Cart cart) {
		if (listOfCarts.keySet().contains(cart.getCartId())) {
			throw new IllegalArgumentException(String.format("Can not create "
					+ "a cart. A cart with the give id (%) already exists", cart.getCartId()));
		}
	...
	public void update(String cartId, Cart cart) {
		if (!listOfCarts.keySet().contains(cartId)) {
			throw new IllegalArgumentException(String.format("Can not update cart. "
					+ "The cart with the give id (%) does not exist", cartId));
		}
	...
	public void delete(String cartId) {
		if (!listOfCarts.keySet().contains(cartId)) {
			throw new IllegalArgumentException(String.format("Can not delete "
					+ "cart. The cart with the give id (%) does not exist", cartId));
		}
